...
function MtpAuthorizer(MtpDcConfigurator, MtpRsaKeysManager, MtpSecureRandom, MtpTimeManager, CryptoWorker) {
    ...
    function mtpSendPlainRequest(dcID, requestBuffer) {
        ...
        var headerArray = new Int32Array(headerBuffer);
        ...
        var requestPromise;
        try {
            requestPromise = fetch(url, {method: 'post', body: requestData}).then((response)=>{
        ...
        return requestPromise.then(
            function (result) {
                ...
                try {
                    ...
                    console.log('MtpAuthorizer.mtpSendPlainRequest: auth_key_id=', auth_key_id);
                    console.log('MtpAuthorizer.mtpSendPlainRequest: msg_id=', msg_id);
                    console.log('MtpAuthorizer.mtpSendPlainRequest: msg_len=', msg_len);
    ...
    function mtpSendReqPQ(auth) {
        var deferredReject = auth.reject;
    ...
    function mtpSendReqDhParams(auth) {
        ...
        mtpSendPlainRequest(auth.dcID, request.getBuffer()).then(function (deserializer) {
            ...
            if (response._ != 'server_DH_params_fail' && response._ != 'server_DH_params_ok') {
                deferredReject(new Error('[MT] Server_DH_Params response invalid: ' + response._))
                return false
            }

            if (!bytesCmp(auth.nonce, response.nonce)) {
                deferredReject(new Error('[MT] Server_DH_Params nonce mismatch'))
                return false
            }

            if (!bytesCmp(auth.serverNonce, response.server_nonce)) {
                deferredReject(new Error('[MT] Server_DH_Params server_nonce mismatch'))
                return false
            }
            ...
            if (response._ == 'server_DH_params_fail') {
                ...
                if (!bytesCmp(newNonceHash, response.new_nonce_hash)) {
                    deferredReject(new Error('[MT] server_DH_params_fail new_nonce_hash mismatch'))
                    return false
                }
                deferredReject(new Error('[MT] server_DH_params_fail'))
                return false
            }
    ...
    function mtpSendSetClientDhParams(auth) {
        var deferredResolve = auth.resolve;
        var deferredReject = auth.reject;
        ...
        CryptoWorker.modPow(gBytes, auth.b, auth.dhPrime).then(function (gB) {
            ...
            mtpSendPlainRequest(auth.dcID, request.getBuffer()).then(function (deserializer) {
                ...
                if (response._ != 'dh_gen_ok' && response._ != 'dh_gen_retry' && response._ != 'dh_gen_fail') {
                    deferredReject(new Error('[MT] Set_client_DH_params_answer response invalid: ' + response._))
                    return false
                }

                if (!bytesCmp(auth.nonce, response.nonce)) {
                    deferredReject(new Error('[MT] Set_client_DH_params_answer nonce mismatch'))
                    return false
                }

                if (!bytesCmp(auth.serverNonce, response.server_nonce)) {
                    deferredReject(new Error('[MT] Set_client_DH_params_answer server_nonce mismatch'))
                    return false
                }
                    ...
                    switch (response._) {
                        case 'dh_gen_ok':
                            ...
                            if (!bytesCmp(newNonceHash1, response.new_nonce_hash1)) {
                                deferredReject(new Error('[MT] Set_client_DH_params_answer new_nonce_hash1 mismatch'))
                                return false
                            }
                            ...
                            deferredResolve(auth)
                            break

                        case 'dh_gen_retry':
                            var newNonceHash2 = sha1BytesSync(auth.newNonce.concat([2], authKeyAux)).slice(-16)
                            if (!bytesCmp(newNonceHash2, response.new_nonce_hash2)) {
                                deferredReject(new Error('[MT] Set_client_DH_params_answer new_nonce_hash2 mismatch'))
                                return false
                            }
                        ...
                        case 'dh_gen_fail':
                            ...
                            if (!bytesCmp(newNonceHash3, response.new_nonce_hash3)) {
                                deferredReject(new Error('[MT] Set_client_DH_params_answer new_nonce_hash3 mismatch'))
                                return false
                            }

                            deferredReject(new Error('[MT] Set_client_DH_params_answer fail'));
                            return false;
                    }
                }, function (error) {
                    deferredReject(error);
