    ...
    MtpNetworker.prototype.sendEncryptedRequest = function (message, options) {
        ...
        return this.getEncryptedMessage(dataWithPadding).then(function (encryptedResult) {
            ...
            try {
                ...
                // TODO (vserhiienko), post does not take options now.
                // requestPromise = fetch(url, { method: 'post', body: requestData, headers: {'Content-Type': 'application/octet-stream', 'Accept': '*/*'} }).then((response) => {
                
                console.log(requestData);
                
                requestPromise = fetch(url, { method: 'post', body: requestData, headers: {'Accept': '*/*'} }).then((response) => {
