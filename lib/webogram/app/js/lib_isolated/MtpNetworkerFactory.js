function MtpNetworkerFactory(MtpDcConfigurator, MtpTimeManager, MtpSecureRandom, Storage, CryptoWorker, $rootScope) {
    ...
    MtpNetworker.prototype.updateSentMessage = function (sentMessageID) {
        ...
        if (sentMessage.container) {
            ...
            // console.log('sentMessage.inner?', sentMessage.inner);
            Array.polymorphicForEach(sentMessage.inner, function (innerSentMessageID) {
                // console.log('innerSentMessageID?', innerSentMessageID);
    ...
    MtpNetworker.prototype.getAesKeyIv = function (msgKey, isOut) {
        ...
        var sha2a = CryptoWorker.sha256Hash(sha2aText);
        // console.log('promises.sha2a?', sha2a);

        sha2bText.set(authKey.subarray(40 + x, 40 + x + 36), 0);
        sha2bText.set(msgKey, 36);
        var sha2b = CryptoWorker.sha256Hash(sha2bText);
        // console.log('promises.sha2a?', sha2b);

        return Promise.all([sha2a, sha2b]).then(function (result) {
            // console.log('Promise.all?promises?result?', result);

            var aesKey = new Uint8Array(32);
            var aesIv = new Uint8Array(32);
            var sha2a = new Uint8Array(result[0]);
            var sha2b = new Uint8Array(result[1]);
    ...
    MtpNetworker.prototype.checkConnection = function (event) {
        ...
        clearTimeoutPromise(this.checkConnectionPromise)
        ...
        this.sendEncryptedRequest(pingMessage, { timeout: 15000 }).then(function (result) {
            ...
            self.checkConnectionPromise = setTimeoutPromise(self.checkConnection.bind(self), parseInt(self.checkConnectionPeriod * 1000))
    ...
    MtpNetworker.prototype.toggleOffline = function (enabled) {
        ...
        if (this.offline) {
            clearTimeoutPromise(this.nextReqPromise)
            delete this.nextReq
            ...
            this.checkConnectionPromise = setTimeoutPromise(this.checkConnection.bind(this), parseInt(this.checkConnectionPeriod * 1000))
    ...
    MtpNetworker.prototype.performSheduledRequest = function () {
        ...
        // console.log('performSheduledRequest: pendingMessages?', this.pendingMessages);
        Array.polymorphicForEach(this.pendingMessages, function (value, messageID) {

            // console.log('performSheduledRequest: value?', value, messageID);            
        ...
        this.sendEncryptedRequest(message).then(function (result) {
            // console.log('sendEncryptedRequest? message', message, 'result', result);
            ...
            self.parseResponse(result.data).then(function (response) {
                ...
                Array.polymorphicForEach(noResponseMsgs, function (msgID) {
            ...
            if (message.container) {
                Array.polymorphicForEach(message.inner, function (msgID) {
    ...
    // https://stackoverflow.com/a/46946573
    // Options can be { timeout } or nothing, currently is ignored by fetch.
    ...
    MtpNetworker.prototype.parseResponse = function (responseBuffer) {
        ...
        // console.log('parseResponse: responseBuffer?', responseBuffer);
    ...
    MtpNetworker.prototype.sheduleRequest = function (delay) {
        ...
        clearTimeoutPromise(this.nextReqPromise)
        if (delay > 0) {
            this.nextReqPromise = setTimeoutPromise(this.performSheduledRequest.bind(this), delay || 0)
        } else {
    ...
    MtpNetworker.prototype.cleanupSent = function () {
        ...
        Array.polymorphicForEach(this.sentMessages, function (message, msgID) {
