...
/* angular for each (not sure if it is fully compatible) */
if (!Array.polymorphicForEach) {
	Array.polymorphicForEach = function (objOrArray, callback) {
    ...
    if (Array.isArray(objOrArray)) {
      ...
      var kk = Object.keys(objOrArray);
      var k;
      for (var i = 0; i < kk.length; i++) {
        k = kk[i];
        callback(objOrArray[k], k);
      }
    }
  }
}

function clearTimeoutPromise(p) {
  // console.log('clearTimeoutPromise', p);
  p && p.cancel();
}

function setTimeoutPromise(f, delay) {
  // console.log('setTimeoutPromise', f, delay);

  var timerId, rejectPromise;
  var promise = new Promise((resolve, reject) => {
      timerId = setTimeout(() => resolve(f()), delay || 0);
...
/* Object.isObject for consistency */
if (!Object.isObject) {
